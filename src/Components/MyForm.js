...
class MyForm extends React.Component {
	...
	applyDates = () => {
		...
		const unix1 = parseInt((new Date(`${date1.y}-${date1.m}-${date1.d}`).getTime() / 1000).toFixed(0));
		const unix2 = parseInt((new Date(`${date2.y}-${date2.m}-${date2.d}`).getTime() / 1000).toFixed(0));
		...
		return (
			<form className='formStyle' onSubmit={this.onSubmit}>
				...
				<div className='form-content'>
					...
					<div className='row'>
						<Filter className='form-control' addRange={this.props.addRange} applyDates={this.applyDates} />
					</div>
					<div className='row'>
